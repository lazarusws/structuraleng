...
function blank_output(){
    ...
    document.getElementById( "allowableBearingCapacity").innerHTML      = "";
...
function flexure_design() {      
    ...
    var sp_mm_1 = Math.min((short_span * 1000 ) / ((As_1 / bar_asmm) -1), 2 * D * 1000, 250);  //Spacing in mm

    var sp_1 = Math.max(sp_mm_1, Math.sqrt(4* bar_asmm/ Math.PI), 20);                         //Checking minimum spacing in mm

    var sp_mm_2 = Math.min(( long_span * 1000 ) / ((As_2 / bar_asmm) -1), 2 * D * 1000, 250) ;

    var sp_2 = Math.max(sp_mm_2, Math.sqrt(4* bar_asmm/ Math.PI), 20);                         //Checking minimum spacing in mm
